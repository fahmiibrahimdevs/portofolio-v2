    ...
    <div className="flex flex-col h-full max-h-[90vh] bg-slate-900">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3.5 bg-slate-900">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2 text-xs bg-slate-950 border border-slate-800 rounded-xl text-slate-100 placeholder-slate-500 focus:bg-slate-950 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all shadow-2xs"
          />
          {search && (
            <button
              type="button"
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-0.5">
          <button
            ...
            className={`px-2.5 py-1 text-[11px] font-semibold rounded-lg whitespace-nowrap transition-all flex items-center gap-1 cursor-pointer ${
        ...
        <div className="flex items-center justify-between text-[11px] text-slate-400 px-1 pt-1 border-t border-slate-800/60">
          <div className="flex items-center gap-2">
            ...
            {selectedInFilteredCount > 0 && (
              <span className="text-[10px] font-bold text-cyan-600 dark:text-cyan-400 bg-cyan-500/10 px-2 py-0.5 rounded-full border border-cyan-500/30">
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-between p-2.5 rounded-xl border text-xs cursor-pointer select-none transition-all ${
                    isChecked
                      ? "bg-cyan-500/10 border-cyan-500 text-cyan-700 dark:text-cyan-300 ring-1 ring-cyan-500/30 shadow-xs"
                      : "bg-slate-950/70 border-slate-800 hover:border-slate-700 text-slate-300 shadow-2xs"
                  }`}
                >
                  <div className="flex items-center gap-2.5 min-w-0 pr-2">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {isChecked && (
                          <span className="text-[9px] font-bold text-cyan-600 dark:text-cyan-400 bg-cyan-500/10 border border-cyan-500/30 px-1.5 py-0.2 rounded flex items-center gap-0.5">
          ...
          {hasMore && (
            <div className="pt-2">
              <button
                ...
                className="w-full py-2 px-3 bg-slate-950 hover:bg-slate-850 border border-slate-800 hover:border-cyan-500/40 text-cyan-600 dark:text-cyan-400 font-semibold text-xs rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-2xs group cursor-pointer"
              >
          ...
          {visibleLimit > INITIAL_LIMIT && sortedSkills.length > INITIAL_LIMIT && (
            <div className="pt-1">
              <button
                ...
                className="w-full py-1.5 px-3 bg-slate-950 hover:bg-slate-850 text-slate-400 hover:text-slate-200 font-medium text-[11px] rounded-xl border border-slate-800 transition-all flex items-center justify-center gap-1 cursor-pointer"
              >
      ...
      <div className="p-4 border-t border-slate-800/80 bg-slate-900/50 flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={onClose}
          className="px-5 py-2 text-xs font-bold text-white bg-cyan-600 hover:bg-cyan-700 rounded-xl shadow-md transition-all cursor-pointer"
        >
          Selesai
        </button>
      </div>
